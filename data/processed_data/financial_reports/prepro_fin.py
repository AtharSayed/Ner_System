...
# Function to load the financial report dataset (including SEC files)
...
# Preprocessing pipeline function for financial reports (SEC filings)
def preprocess_financial_reports(file_path):
...
# List of files to process (financial reports such as SEC 10-K filings)
files_to_process = [
    r'F:\M.Tech_CollgeMaterials\Ner_System\data\rawdata\financial_reports\Financial-QA-10k.csv',  
]

# Process each file and save the preprocessed output
for file_path in files_to_process:
    # Process the file
    processed_df = preprocess_financial_reports(file_path)
    
    # Create a folder for processed data if it doesn't exist
    os.makedirs('data/processed_data/financial_reports', exist_ok=True)
    
    # Extract the base name from the file path (remove extension)
    base_name = os.path.basename(file_path).split('.')[0]
    
    # Modify the output file path for the processed data
    output_file_path = os.path.join('data/processed_data/financial_reports', f'{base_name}_processed.csv')
